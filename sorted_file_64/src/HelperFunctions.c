...
Index indexIncr(Index index, int size){
	...
	if (index.recordIndex == lastRecordIndex)
	{	//and the block is the last block
		if (index.blockIndex == size - 1)
		{//then we cannot increase it without getting out of borders
